...
select  sinif , count(*) as sinif_ogrenciSayisi from ogrenci
group by sinif;

-- Task 3: Her sınıftaki erkek ve kız öğrenci sayısını bulun.(cinsiyet ve sinifa göre grupla)
select  sinif, cinsiyet , count(*) as cinsiyet_ogrenciSayisi from ogrenci
group by sinif , cinsiyet;
-- Task 4: Her türden kaç tane kitap olduğunu listeleyiniz.
select  turno , count(idno) as kitap_sayisi from kitap
group by turno; 

-- Task 5: Her türdeki en fazla sayfa sayısı olan kitapları listeleyiniz.
select  turno , max(sayfasayisi) as kitap_sayisi from kitap
group by turno;


-- Task 6: Her türdeki en az sayfa sayısı olan kitapları listeleyiniz.
select  turno , min(sayfasayisi) as kitap_sayisi from kitap
group by turno;

-- Task 7***: Her türden kaç tane kitap olduğunu listeleyiniz.(Tür isimleri de olsun)
select kitap_turu ,(select count(turno) from kitap where kitap.turno=islemler.turno) as adet_sayisi from islemler;
